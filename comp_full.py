...
vocab_dataloader = DataLoader(VocabDataset(vocab, max_seq_len, assembled_model.encode_input), batch_size=512, collate_fn=VocabDataset.collate_fn, shuffle=True)
...
train_dataloader = DataLoader(dataset, batch_size=1, collate_fn=collate_fn)#, num_workers=8, prefetch_factor=4)#, num_workers=8, prefetch_factor=18, shuffle=True)
...
log_dir = os.path.join('.clogs/all_params', f'prog gen 50% L2 + cross')
