...
def __build_tree__(prog):
    ...
    program_mask = []
    for line in prog:
        if line[0] in RASP_OPS:
            program_mask.append(1)
            args = line[1:4]
            args = list(filter(lambda x: (not x.startswith('LAM')) and (not x.startswith('PRED')) and x!='NA', args))
            return_var = line[-1] # the name of this instruction
            tree.add_node(return_var, args, meta=line[:4])
        else:
        ...
        for line in depth_set:
            if line is not None:
                ...
                for instruction in line:
                    if instruction in token_weight:
                        score += token_weight[instruction]
                scores.append(score)
        # print(depth_set)
        # print(scores)
