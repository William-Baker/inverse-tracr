...
class TrainerModule:
    ...
    def __init__(self, model_name, exmp_batch, max_iters, dataset, lr=1e-3, warmup=100, seed=42, **model_kwargs):
        ...
        self.model = Decoder(num_classes=sum(src_dataset.segment_sizes), **model_kwargs)
        # Prepare logging
    ...
    def get_loss_function(self):
        ...
        def calculate_loss(params, rng, batch, train):
            ...
            loss = 0
            for i, seg_size in enumerate(self.seg_sizes):
                loss += optax.softmax_cross_entropy_with_integer_labels(logits[:, :time_steps, ptr:ptr + seg_size], labels[:, :time_steps, i])
                ptr += seg_size
    ...
    def create_functions(self):
        ...
        def verbose_step(state, batch, step):
            ...
            loss = []
            for i, seg_size in enumerate(self.seg_sizes):
                loss.append(np.array(optax.softmax_cross_entropy_with_integer_labels(logits[:, :time_steps, ptr:ptr + seg_size], labels[:, :time_steps, i])))
                ptr += seg_size
