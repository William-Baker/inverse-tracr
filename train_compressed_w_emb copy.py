...
def calculate_loss(params, batch):
    ...
    output = compressed_assembled_model.forward(params, encoded_tokens)
...
state = LevenbergMarquardt(calculate_loss, jit=False, verbose=True)
params = unfreeze(compressed_assembled_model.params)
...
for epoch in range(EPOCHS):
    with tqdm(total=len(train_dataloader), unit='batch') as tepoch:
        ...
        for idx, batch in enumerate(train_dataloader):
            ...
            new_params = state.run(params, (encoded_tokens, target_outs, logits))
...
log_dir = os.path.join('.clogs', f'L2 leven')
