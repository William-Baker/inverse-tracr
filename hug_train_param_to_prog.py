...
class TrainerModule:
    ...
    def get_accuracy_function(self):
        def accuracy(logits, labels, loss_mask):
            ...
            relevant_classes = classes[:, :time_steps, :] * loss_mask
            relevant_labels = labels[:, :time_steps, :] * loss_mask
            relevant_labels += 1 - loss_mask # ensure the masked out values are different
            acc = relevant_classes == relevant_labels
            acc = acc.sum() / (loss_mask * relevant_labels.shape[2])
            return acc
        return accuracy
...
dataset = WrappedDataset('.data/iTracr_dataset/', PROG_LEN, 40)
...
#%%
trainer = TrainerModule(model, 'PARAM_v3_1_GPT2',#'no mean shuffled inputs pose in hid',#f'11 big lr: {LEARNING_RATE} bs: {batch_size} epcs: {max_epochs}', 
                        next(test_it), 
                        num_train_iters, 
                        dataset=src_dataset, 
                        lr=LEARNING_RATE)
...
#trainer.load_model(log_dir='PARAM_v2_2_GPT2')
...
if output != '':
    ...
    if usage > maxUsage:
        maxUsage = usage
        maxUsage_unit = usage_unit
    #usage_str = str(usage) + usage_unit
    maxUsage_str = str(maxUsage) + maxUsage_unit
print(usage)
