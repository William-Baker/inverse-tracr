...
def assemble_craft_model(
  ...
  def get_compressed_compiled_model():
    transformer = compressed_model.CompressedTransformer(model_config, embedding_size=len(residual_space.basis))  # TODO get embedding size properly from craft model
    embed_modules = _make_embedding_modules(
        residual_space=residual_space,
        tokens_space=tokens_space,
        indices_space=indices_space,
  ...
  compressed_params = compressed_forward.init(jax.random.PRNGKey(0), jnp.array([[1, 2, 3]]))
  ...
  for key in compressed_params:
    if "transformer" in key:
      for par in compressed_params[key]:
        compressed_params[key][par] = np.zeros_like(compressed_params[key][par])
  ...
  for component_name, component_param_dict in params.items():
    compressed_equivalent = component_name.replace('transformer', 'compressed_transformer')
    assert compressed_equivalent in compressed_params
    for sub_name, sub_params in component_param_dict.items():
      compressed_params[compressed_equivalent][sub_name] = np.copy(sub_params) # create a copy for the compressed model
  # make the compression weights identity for testing
  compressed_params['compressed_transformer']['w_emb'] = np.eye(*compressed_params['compressed_transformer']['w_emb'].shape)

  params = jax.tree_util.tree_map(jnp.array, params)
  compressed_params = jax.tree_util.tree_map(jnp.array, compressed_params)

  assembled_transformer = AssembledTransformerModel(
  ...
  assembled_compressed_transformer = AssembledTransformerModel(
...
print(f"Runnning {prog_name} with input {input_seq}")
prog_out = compressed_assembled_model.apply(formatted_input).decoded
print(f"Program outputs: {prog_out}")
